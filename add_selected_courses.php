...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['selectCourses']) && $is_page_refreshed) {
        ...
        $sql = "SELECT course_id, course_title, course_credit, course_faculty, section, lab FROM courses WHERE course_id = '$id' and section = '$section'";
        ...
        if(mysqli_num_rows($searchSql) == 0){
            if (mysqli_num_rows($conSql) < 4){
                $insertSql = "INSERT INTO selected_courses (course_id, course_title, course_credit, lab, course_faculty, section) 
                VALUES ('$courseId', '$courseTitle', '$courseCredit', '$lab', '$courseFaculty', '$courseSection')";
                mysqli_query($conn, $insertSql);

                $removeSql = "DELETE from courses where section = '$courseSection' and course_id = '$courseId'";
